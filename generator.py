...
def create_combined_image(directory_path, base_name, produce_2x):
  ...
  if produce_2x:
    combined_image_2x_path = os.path.join(directory_path,
                                          f"{base_name}@2x.png")
    combined_image_2x = combined_image.resize((cell_width * columns // 2, cell_height * rows // 2), Image.ANTIALIAS)
    combined_image_2x.save(combined_image_2x_path)
    print(
        f"2x Combined grid image saved successfully at {combined_image_2x_path}."
    )
